...
class SaleController extends Controller {
	...
	public function index()
	{
		...
		return view('sale.index',compact('items','sections'))
			->with('sale', $sales)
    ...
    {
        $q = $request->input( 'q' );
        $item = Item::where('item_name','LIKE','%'.$q.'%')->orWhere('description','LIKE','%'.$q.'%')->get();
        if(count($item) > 0)
            return view('sale.index')->withDetails($item)->withQuery ( $q );
        else return view ('sale.index')->withMessage('No Details found. Try to search again !');
    }
	...
	public function store(SaleRequest $request)
    {
        ...
        $customer->name = Input::get('c_name');
        $customer->save();
